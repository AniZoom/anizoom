...
const { data: data } = (await useFetch("/api/anime/" + id)) as Record<
  string,
  any
>;
...
const externalLinks = anime.externalLinks.filter((e: { site: string }) => {
...
const streamingEpisodes =
  sortEpisodes(anime?.streamingEpisodes)?.slice(0, 6) || [];

const seoDescription = fixSeoDescription(
      ...
      <div class="col px-0 pb-5">
        ...
        <div class="px-2 pt-4 pt-lg-5 px-lg-5 px-xl-5 w-100">
          <ComponentAnimeMenu
            :anime-id="String(id)"
            :slug="String(slug)"
            :episodes="streamingEpisodes[0] ? true : false"
          />
          <div class="pt-4 px-0">
            ...
            <h6 v-if="anime.title.english" class="mb-1">
              {{ anime.title.english }}
            </h6>
            <h6 v-if="anime.title.native" class="mb-1">
              {{ anime.title.native }}
            </h6>
            <div class="d-flex align-items-center position-relative">
              <div class="stars d-flex align-items-center" style="height: 25px">
                ...
                <img
                  ...
                  :style="{
                    'clip-path':
                      'inset(0px ' + (100 - anime.averageScore) + '% 0px 0px) ',
                  }"
                />
              </div>
              <small class="ms-2 mb-0 text-white">{{
          ...
          <div
            id="details"
            class="pt-3 pb-4 d-flex align-items-start anime-row mx-0 flex-wrap px-0"
          >
            <img
              id="cover"
              :src="anime?.coverImage?.extraLarge"
              class="col-12 col-sm-12 col-md-4 col-lg-4 col-xl-3 img-fluid px-0"
              style="max-width: 400px"
              :alt="anime.title.romaji"
              :title="anime.title.romaji"
            />
            <div
              class="col-12 col-sm-12 col-md-8 col-lg-8 col-xl-9 pt-4 pt-md-0 px-0 ps-md-4"
            >
              ...
              <div
                class="d-flex justify-content-start align-items-start anime-row flex-wrap m-0"
              >
                <h6
                  v-if="anime.status"
                  class="mb-2 col-12 col-sm-6 col-md-6 col-lg-6 col-xl-4 mt-0 p-0 text-capitalize"
                >
                ...
                <h6
                  ...
                  class="mb-2 col-12 col-sm-6 col-md-6 col-lg-6 col-xl-4 mt-0 p-0"
                >
                  <span class="text-primary">Release Date:</span>&nbsp;
                  {{
                ...
                <h6
                  v-if="getStudios(anime.studios)"
                  class="mb-2 col-12 col-sm-6 col-md-6 col-lg-6 col-xl-4 mt-0 p-0"
                >
                  <span class="text-primary">Studios:</span>&nbsp;
                  {{ getStudios(anime.studios) }}
                </h6>
                <h6
                  v-if="getProducers(anime.studios)"
                  class="mb-2 col-12 col-sm-6 col-md-6 col-lg-6 col-xl-4 mt-0 p-0"
                >
                  <span class="text-primary">Producers:</span>&nbsp;
                  {{ getProducers(anime.studios) }}
                </h6>
              </div>
              <div v-if="anime.genres">
                <h6 class="mb-2">
                  ...
                  <span v-for="(genre, i) of anime.genres" :key="i">
                    <span
                      class="badge bg-secondary align-middle mx-1 fw-normal"
                      >{{ genre }}</span
                    >
                  </span>
                </h6>
                ...
                <div class="col-12 col-lg-12 col-xl-6">
                  ...
                  <div class="ratio ratio-16x9">
                    <iframe
                      ...
                      width="1280"
                      height="720"
                      :src="'https://youtube.com/embed/' + anime.trailer.id"
                      frameborder="0"
          ...
          <div v-if="anime?.characters?.edges[0]" id="characters" class="pb-4">
            ...
            <div
              class="d-flex justify-content-start align-items-start anime-row flex-wrap gx-0 gx-lg-3 gy-1 m-0"
            >
              <div
                v-for="(c, i) of anime.characters.edges"
                :key="i"
                class="col-12 col-lg-6 col-xl-6 col-xxl-4 mb-3"
              >
                <div
                  class="d-flex align-items-start anime-row flex-wrap row-cols-auto g-2 bg-secondary rounded m-0"
                >
                  ...
                  <div class="col pe-0 mt-0">
                    <img
                      ...
                      class="img-fluid rounded-start"
                      style="max-height: 90px"
                      :alt="c.voiceActors[0]?.name.userPreferred"
                      :title="c.voiceActors[0]?.name.userPreferred"
                    />
                  </div>
                </div>
              </div>
          ...
          <div v-if="streamingEpisodes[0]" id="episodes">
            ...
            <div
              class="d-flex justify-content-start align-items-start anime-row flex-wrap m-0 g-2"
            >
              <template v-for="(ep, i) of streamingEpisodes" :key="i">
                <a
                  class="col-6 col-sm-4 col-md-4 col-lg-4 col-xl-3 col-xxl-2 mb-2 text-muted"
                  :href="ep.url"
                >
                  <div class="bg-dark rounded">
                    <div
                      class="overflow-hidden position-relative rounded rounded-bottom-0"
                    >
                      <img
                        class="img-fluid rounded rounded-bottom-0 scale-on-hover w-100"
                        :src="ep.thumbnail"
                        :alt="ep.title"
                        :title="ep.title"
                      />
                      <span
                        class="position-absolute top-50 start-50 translate-middle mb-0 invisible pe-none text-white"
